...
function applyOperationalFlowUpdates(target) {
  ...
  setIngredient('ing_fraldinha', {
    operationalUnit: {
      label: 'Porção de lanche',
      type: 'porção',
      instrument: 'Balança',
      quantity: 150,
      unit: 'g',
      storage: 'Refrigerado',
      notes: 'Porção operacional usada no Gyros de fraldinha.'
    },
    operationFlow: [
      { stage: 'recebimento', title: 'Chega como carne preparada ou peça', description: 'A compra pode variar por fornecedor; internamente o item é Fraldinha Bovina preparada.' },
      { stage: 'manipulacao', title: 'Preparar para montagem', description: 'Ajustar corte e ponto operacional antes de porcionar.' },
      { stage: 'porcionamento', title: 'Porção do lanche', description: 'Separar porções de 150g para uso no Gyros.' },
      { stage: 'uso', title: 'Base do Alpha', description: 'Entra no Alpha Gyros e adicionais de carne.' }
    ]
  });
...
function applyUrlStateFromSearch() {
  ...
  if (['dashboard', 'catalog', 'ledger', 'operation', 'resources'].includes(page)) state.page = page;
...
function renderProcurementNode(node = {}, context = 'produto') {
  ...
  const recipeOutputs = node.refType === 'recipe' ? recipeOutputPortions(record, node) : [];
  const recipeCoverage = node.refType === 'recipe' ? recipeUsageCoverageLabel(record, node) : '';
  const recipeCanonicalRows = node.refType === 'recipe' ? recipeCanonicalCostRows(record) : [];
  return `<div class="procurement-card">
    <div class="procurement-head">
    ...
    <div class="procurement-steps">
      ...
      ${node.refType === 'recipe' ? renderProcurementStep('Rendimento da receita', recipeYieldLabel(record), record.storage || '') : ''}
      ${node.refType === 'recipe' && recipeYieldSecondaryLabel(record) ? renderProcurementStep('Equivalente do lote', recipeYieldSecondaryLabel(record), 'referência para kg/L') : ''}
      ${node.refType === 'recipe' ? renderProcurementStep('Unidade canônica ERP', canonicalErpUnitForRef('recipe', record.id), recipeCanonicalSummaryLabel(record)) : ''}
      ${recipeCanonicalRows.map(row => renderProcurementStep(`Custo por ${canonicalUnitDisplay(row.unit)}`, brl(row.cost), `${decimal(row.qty)} ${canonicalUnitDisplay(row.unit)} no lote`)).join('')}
    </div>
    ${node.refType === 'recipe' ? `<div style="margin-top:10px;">
      <div class="small muted">Saídas do lote e cobertura</div>
      <div class="procurement-steps" style="margin-top:8px;">
        ${recipeCoverage ? renderProcurementStep('Cobre esta ficha', recipeCoverage, 'sem considerar estoque real') : ''}
        ${recipeOutputs.map(output => renderProcurementStep(output.label || 'Saída operacional', outputPortionCountLabel(record, output), `${formatQtyWithUnit(output.qty, output.unit || record.yieldUnit)} cada`)).join('')}
      </div>
    </div>` : ''}
...
function renderProductProcurementFlow(c) {
  ...
  return `<section>
    <div class="catalog-section-head"><div><h3>Da compra ao produto vendido</h3><p>Camadas reais do fluxo: produto, preparos e insumos de compra sem achatar a ficha técnica.</p></div></div>
...
function renderProductSummaryTab(c, basePortions, addonGroups) {
  ...
  return `<div class="stack">
    ...
    <div class="info-grid">
      ...
      <div class="info-cell"><div class="k">Unidade canônica ERP</div><div class="v">${escapeHtml(canonicalErpUnitForRef('product', p.id))}</div><div class="small muted">item de catálogo</div></div>
